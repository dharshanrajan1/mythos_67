...
export async function handleOnboardingReply(userId, text) {
    const session = await prisma.onboardingSession.upsert({
        where: { userId },
        update: {},
        create: { userId },
    ...
    if (session.currentStep === "BODY_STATS") {
        ...
        if (!parsed.age || !parsed.height_cm || !parsed.weight_kg) {
            return {
                replyText: "need a little more here. send age, height, and current weight together, like `28 male 5'10 180 lb`!",
            };
        }
    ...
    if (session.currentStep === "ACTIVITY") {
        ...
        if (parsed.exercise_days_per_week === null) {
            return {
                replyText: "just send the number of days you usually train each week, like `4 days` or `three times a week`!",
            };
        }
        await prisma.profile.upsert({
            where: { userId },
...
async function parseGoal(userId, text) {
    const conversation = await getRecentConversationText(userId);
    try {
        return await createJsonResponse({
            instructions: "extract the user's primary fitness goal from onboarding. map body recomposition goals to maintain unless the user clearly prioritizes gain or loss. keep note short if helpful, like 'recomp'.",
            content: [
                {
                    type: "input_text",
...
async function parseBodyStats(userId, text) {
    const conversation = await getRecentConversationText(userId);
    try {
        return await createJsonResponse({
            instructions: "extract body stats from the user's onboarding reply. return height in centimeters and weight in kilograms. if a field is missing, return null. do not guess age, height, or weight.",
            content: [
                {
                    type: "input_text",
                    text: `recent conversation:\n${conversation}\n\ncurrent onboarding reply:\n${text}`,
...
async function parseActivity(userId, text) {
    const conversation = await getRecentConversationText(userId);
    try {
        return await createJsonResponse({
            instructions: "extract how many days per week the user usually exercises or trains. convert english words like 'three' into digits. if the user gives a range, choose the most representative single number. also return a short activity note if useful.",
            content: [
                {
                    type: "input_text",
                    text: `recent conversation:\n${conversation}\n\ncurrent onboarding reply:\n${text}`,
...
async function parseDiet(userId, text) {
    const conversation = await getRecentConversationText(userId);
    try {
        return await createJsonResponse({
            instructions: "extract diet preferences, allergies, and any extra notes from the user's onboarding reply. keep lists short and normalized.",
            content: [
                {
                    type: "input_text",
                    text: `recent conversation:\n${conversation}\n\ncurrent onboarding reply:\n${text}`,
...
function parseBodyStatsFallback(text) {
...
function parseActivityFallback(text) {
    const lower = text.toLowerCase();
...
function parseDietFallback(text) {
    const lower = text.toLowerCase();
