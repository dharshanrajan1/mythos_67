...
export async function resolveNutritionEstimate(input) {
    ...
    if (normalizedExtracted.needs_clarification || normalizedExtracted.items.length === 0) {
        return {
            ...
            replyText: formatClarificationReply(normalizedExtracted.clarification_question ?? "quick check: what exactly was in that meal?"),
        };
    }
    const items = [];
...
function pickBestSource(input) {
    ...
    return {
        ...
        foodSource: bestExternal.foodSource === "BRANDED" ? "BRANDED" : "USDA",
...
async function extractMeal(input) {
...
function inferGrams(quantity, unit) {
    if (!quantity) {
        return 100;
    }
...
function fallbackExtractMeal(text) {
    const parts = text
        .split(/,| and /i)
...
function normalizeExtractedMeal(extracted) {
    if (extracted.items.length <= 1) {
        return extracted;
    }
    const lowerMealLabel = extracted.meal_label.toLowerCase();
    const looksLikeSingleBrandedItem = extracted.items.length > 1 &&
...
function mergeItemsIntoSingleMenuItem(extracted) {
    const total = extracted.items.reduce((acc, item) => {
