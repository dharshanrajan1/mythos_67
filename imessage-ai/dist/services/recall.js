...
async function classifyRecallQuery(userId, text) {
    const lower = text.toLowerCase();
    if (lower.includes("what did i eat") ||
    ...
    try {
        ...
        return await createJsonResponse({
            instructions: "decide whether the user is asking about previously logged meals, foods, nutrition totals, or anything that should be answered from recent meal log memory rather than logged as a new meal.",
            content: [
                {
                    type: "input_text",
...
async function answerRecallQuestion(userId, question, context) {
    try {
        const conversation = await getRecentConversationText(userId);
        const result = await createJsonResponse({
            instructions: "you are sam, a chill nutrition coach texting the user. answer the user's question only from the provided meal log context from the last 24 hours. do not invent meals or numbers. if the answer is uncertain, say that briefly. keep it concise, helpful, and natural.",
            content: [
                {
                    type: "input_text",
                    text: `recent conversation:\n${conversation}\n\nmeal log context:\n${context}\n\nuser question:\n${question}`,
...
function buildRecallContext(mealEvents) {
    const lines = [];
    let totalCalories = 0;
    let totalProtein = 0;
    let totalCarbs = 0;
    ...
    for (const event of mealEvents) {
        ...
        lines.push(`${formatTimestamp(event.mealTime)}: ${itemParts.join("; ")}`);
    }
    lines.push(`totals: ${Math.round(totalCalories)} cal, ${Math.round(totalProtein)}p, ${Math.round(totalCarbs)}c, ${Math.round(totalFat)}f`);
    return lines.join("\n");
}
function fallbackRecallAnswer(question, context) {
