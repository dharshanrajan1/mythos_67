...
export async function maybeHandleCorrection(input: {
  ...
  if (!targetItem) {
    return {
      handled: true,
      replyText:
        "i can fix it, but i need the meal name. say something like `change spicy deluxe to 34g protein and 26g fat`.",
    };
  }
  ...
  return {
    ...
    replyText: normalizeCoachText(
      `got it! i updated ${targetItem.nameRaw} to ${Math.round(nextValues.calories ?? 0)} cal, ${Math.round(nextValues.proteinG ?? 0)}p, ${Math.round(nextValues.carbsG ?? 0)}c, and ${Math.round(nextValues.fatG ?? 0)}f. ${
        goal && rollup
...
async function parseCorrection(userId: string, text: string) {
  const conversation = await getRecentConversationText(userId);

  try {
    return await createJsonResponse<ParsedCorrection>({
      instructions:
        "determine whether the user is correcting a previously logged meal item. extract any explicit target item name and any nutrition numbers they want changed. if the user is giving revised nutrition facts for the last mentioned meal, set is_correction true even if they do not say the meal name explicitly.",
      content: [
        {
          type: "input_text",
...
function parseCorrectionFallback(text: string): ParsedCorrection {
...
function matchMacroNumber(text: string, pattern: RegExp) {
